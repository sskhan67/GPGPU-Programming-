...
# def contraction_list_to_code(contraction_terms, vec_state):
# 	return_buf = []
# 	for each_term in contraction_terms:
# 		# Loop Over Sums, matrix_terms, and multiply it by coeff
# 		sums = each_term.get_sum_objs()
# 		for i in range(len(sums)):
# 			return_buf += [ i *'\t' + sum_obj_to_code(sums[i]) ]
# 		if len(return_buf) > 0:
# 			# there are summation loops
# 			if 'for' in return_buf[-1]:
# 				return_buf += [ (return_buf[-1].count('\t') +1 ) * '\t' + generate_contraction_line(each_term, vec_state) ]
# 			else:
# 				return_buf += [ return_buf[-1].count('\t')  * '\t' + generate_contraction_line(each_term, vec_state) ]

# 		else:
# 			# No summation loop at all
# 			return_buf += [ generate_contraction_line(each_term, vec_state) ]
...
def computed_ci_state_to_code( state_obj, func_name ):
	'''Take a computed CI state, generate a multiline long string of Python code'''
	return_buf =  "def " + func_name + "( cisd_amp_obj , F_mat , V_mat ):\n"
	...
	if len(terms) > 0:
		...
		# 
		# 1) Initialize all the matrix terms, new amp matrix needed in this scope.
		...
		# 1) Get Necessary Matrix_like Terms from CISD Amplitude Object
		# mat_terms = terms[0].get_mat_term()
		# for mat_term in mat_terms:
		# 	if matrix_term_to_code(mat_term) != None:
		# 		return_buf = add_lines_to_buffer( return_buf, matrix_term_to_code(mat_term) )

		# #
		# # 2) Get Return vector_state
		# vec_state   = terms[0].get_vec_state()
		# lines = vec_state_to_code(vec_state)
		# return_buf = add_lines_to_buffer(return_buf, lines)


		# # Find out how many '\t' are need for Contraction Loops
		# if 'for' in lines[-1]:
		# 	prefix = '\t' * ( lines[-1].count('\t') + 2 )
		# else:
		# 	prefix = '\t'
		# # print([prefix])

		# #
		# # 3) Get the contraction relation objects for all terms.
		# contraction_terms = state_obj.get_contraction_relation()
		# lines = contraction_list_to_code(contraction_terms, vec_state)
		# return_buf = add_lines_to_buffer(return_buf, lines, prefix)


		# #
		# # 4) Code suffix, update certain parts of the amplitudes and return
		# lines = generate_suffix(vec_state)
		# return_buf = add_lines_to_buffer(return_buf, lines)
		...
		# This state is zero (killed by operators)
		# print("This State is Empty!")
		return None
...
# 	ham_ref  = state([ concatenate(ham, ref_state) ])
# 	ham_ref_return = compute_state(ham_ref)
# 	# print("h|R> =", ham_ref_return.return_info() )
# 	computed_ci_state_to_code(ham_ref_return, 'h01_ref')


# 	ham = h2()
# 	# single state
# 	double_state = get_double_state()
# 	ham_double   = state([ concatenate(ham, double_state) ])
# 	ham_double_return = compute_state(ham_double)
# 	# print("h|S> =", ham_single_return.return_info() )
# 	computed_ci_state_to_code(ham_double_return, 'h02_double')
...
# 	computed_ci_state_to_code(ham_double_return, 'h03_double')
