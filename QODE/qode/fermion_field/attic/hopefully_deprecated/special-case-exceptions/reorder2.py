...
def reorder_e_e(Ebj,Eai,phase=1):
	...
	if i==j or a==b:  raise OpPdtIsZero()
	if j<i:
		i,j = j,i
		phase *= -1
		was_ordered = False
	if b<a:
		...
		phase *= -1
		was_ordered = False
	if was_ordered:  raise OpPdtIsOrdered()
	return [[excitation(b,j), excitation(a,i), phase]]
...
def reorder_e_d(Ebj,Dai,phase=1):
	...
	raise OpPdtIsOrdered()
	return [[Ebj, Dai, phase]]	# theoretical return value
...
def reorder_fo_fo(Flk,Fji,phase=1):
	...
	if j==l or i==k:
		...
		ordered_ops = []
		if l<j or k<i:		# could skip this outer test, but would generate two cancelling terms if already ordered correctly
...
def reorder_fv_fv(Fdc,Fba,phase=1):
	...
	if b==d or a==c:
		...
		ordered_ops = []
		if d<b or c<a:		# could skip this outer test, but would generate two cancelling terms if already ordered correctly
			if b==c:  ordered_ops += [[vrt_rearrange(d,a), +phase]]
			if d<b:
				b,d = d,b
				phase *= -1
