...
for line in lines[1:]:
	...
	if line.rstrip():
		...
		output.write("IDX_{} = Indices(\"{}\", bound=\"No\")\n".format("".join(occs), fields[-3]))
		output.write("{}, = IDX_{}()\n".format(",".join(occs), "".join(occs)))		# trailing comma needed for 1-tuples
		output.write("\n")
		output.write("IDX_{} = Indices(\"{}\", bound=\"Nv\")\n".format("".join(vrts), fields[-2]))	# trailing comma needed for 1-tuples
		output.write("{}, = IDX_{}()\n".format(",".join(vrts), "".join(vrts)))
		output.write("\n")
		output.write("IDX_{} = Indices(\"{}\", bound=\"NMol\")\n".format("".join(mols), fields[-1]))	# trailing comma needed for 1-tuples
		output.write("{}, = IDX_{}()\n".format(",".join(mols), "".join(mols)))
		output.write("\nexpression = Sum(IDX_{})(Sum(IDX_{},IDX_{})({}))\n".format("".join(mols),"".join(occs),"".join(vrts),summand))
