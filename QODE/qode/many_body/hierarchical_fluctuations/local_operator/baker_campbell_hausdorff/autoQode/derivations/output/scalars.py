			...
			for indices in (self.bottom, self.top):
				for n in range(len(indices)):
					...
					if indices[n] in old:
						...
						#sys.stdout.write(str(id(hmm))+"->"+str(id(indices[n]))+"  ")
		#print
		return self
	...
	def __str__(self):
		...
		if len(self.bottom)>0:
			...
			for index in self.bottom:
				string += str(index)		# Sort of assumes this is being called from inside print(<instance of Sum>)
			string += "}"
		if len(self.top)>0:
			string += "^{"
	...
	def code(self):
		...
		if len(self.bottom)>0:
			for i in range(len(self.bottom)):
				indices += [ self.bottom[i].code(), self.top[i].code() ]
		return [name, indices]
