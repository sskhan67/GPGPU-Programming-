...
#class block_restrictor(object):
#	def __init__(self):
#		self.restrict = True
#	def __call__(self,restrict=None):
#		if restrict is not None:  self.restrict = restrict
#		return self.restrict
#restrict_blocks = block_restrictor()		# a singleton which can be modified from elsewhere and read here.
...
class Index(object):
	...
	def __init__(self, index_set, relationships, block=None):
		...
		for relate,other in relationships:
			if other in self.index_set.indices:
				other.add_relationship(reciprocal(relate),self,reciprocate=False)
		self.letter = ''				# volatile, only meainingful over the course of a single "print" (general output) of an index set
		self.block = block	# could be replaced by an outer index indicating which block this index is in
		self.blocked_indices = []	# Which indices need to be updated when block indices merge, always set at same time as .block of members
		#print(id(self))
	def compare(self, other):
		if other is self:  return '='
		if self.block is not None:
			result = self.block.compare(other.block)
			if result!='=':  return result			# explicit relationship only valid if indices definitively in same block (or blocks not defined)
		result = None
		...
		return result
	def add_relationship(self, relate, other, reciprocate=True):
		#print(relate,other,reciprocate)
		#print("in    "+str(self.relationships)) 
		added = False
		for n,(rel,idx) in enumerate(self.relationships):
			if idx is other:
				#print("found it!",rel)
				added = True
				if   rel==relate:
					...
					if   relate=='<' or relate=='=':  self.relationships[n] = (relate,other)
					else:  raise Exception("conflicting index relationships")
				elif rel=='>=':
					if   relate=='>' or relate=='=':  self.relationships[n] = (relate,other)
					else:  raise Exception("conflicting index relationships")
				elif rel=='!=':
					...
					#print(rel,relate)
					raise Exception("conflicting index relationships")
		if not added:
			self.relationships += [(relate,other)]
		#print("out   "+str(self.relationships)) 
		if reciprocate and other in self.index_set.indices:  other.add_relationship(reciprocal(relate),self,reciprocate=False)
	...
	@staticmethod
	def inherit_relationships(parents,to_update):
		...
		for parent in parents:
			if parent.block not in blocks:  blocks += [parent.block]
		if len(blocks)==1:
			block = blocks[0]					# might be None now
		else:
			block,to_update = Index.inherit_relationships(blocks,to_update)		# if the indices are equal, then there blocks must be too.  view as implicit delta.  simplifes things to take care of first.  equality checked here.
		...
		for parent in parents:
			...
			blocked_indices += parent.blocked_indices
		child = index_set._new_index([],block)
		...
		for index in blocked_indices:
			for parent in parents:
				index.replace_in_relationships(parent,child)
		if block is not None:
			block.replace_in_relationships(parents[0],child)	# replace one parent with child in blocked_indices list of block
			for parent in parents[1:]:
				block.replace_in_relationships(parent,None)	# and effectively delete other parents, since all now merged into child
		for parent in parents:
		...
		index_replacements = [(parent,child) for parent in parents]
		updated = []
		for item in to_update:
			updated += [copy(item, index_replacements)]
	...
	def replace_in_relationships(self, old, new):
		...
		if self.block is old:
			self.block = new
			new.blocked_indices += [self]
		for n,index in enumerate(self.blocked_indices):
			if index is old:
				self.blocked_indices[n] = new
		temp = self.blocked_indices
		self.blocked_indices = []
		for item in temp:
			if item is not None:  self.blocked_indices += [item]
	def __str__(self):
		# volatile, only meainingful over the course of a single "print" (general output) of an index set
		if (self.letter is not '') and (self.block is not None):  return self.letter + "_" + str(self.block)	      # block letter should be resolved by the time you want to print this
		else:                                                     return self.letter
	def code(self):
		# volatile, only meainingful over the course of a single "print" (general output) of an index set
		if (self.letter is not '') and (self.block is not None):  return [self.letter , str(self.block)]	      # block letter should be resolved by the time you want to print this
		else:                                                     return [self.letter , None]
...
class Indices(object):
	...
	def _new_index(self, relationships, block=None):
		index = Index(self,relationships,block)
		self.indices += [index]
		return index
	...
	def copy(self, index_replacements):						# purposely not called _copy because not to be confused with things derived from indexed_base
		...
		for index in self.indices:  new._new_index(index.relationships,index.block)		# relationships of new indices all point to old set
	...
	def assign_letters_cased(self):
		""" all indices will be assigned letters. which letter and which restrictions are printed depends on order, but meaning of result does not, so can reorder indices before this """
		...
		for index in self.indices:
			...
			for n,(relate,other) in enumerate(index.relationships):
				if (str(other) is not '') and ((index.block is None) or (index.block.compare(other.block)=='=')):	# don't print explicit relationships if blocks differ b/c that is already in outer sum
					keep = True
...
def case_index(n, cases_so_far):
	...
	for index_set in cases_so_far:
		...
		for other in index_set.indices[:n]:			# search over indices already handled (i.e., casing is order dependent!)
			comparison = index.compare(other)
			#print(comparison)
			if comparison not in ('<','=','>'):					# only need multiple cases if relationship not well defined
				if   comparison is None:  more_relationships = [('<',other), ('=',other), ('>',other)]	# there are three concrete possibilities
				elif comparison=='<=':    more_relationships = [('<',other), ('=',other)]
				elif comparison=='>=':    more_relationships = [('=',other), ('>',other)]
				elif comparison=='!=':    more_relationships = [('<',other), ('>',other)]
		...
		if list_of_relationship_lists is None:
			...
			for relationships in list_of_relationship_lists:
				...
				#print("executing case", relationships,"  to modify=",cased_index_set.indices[n])
				for relate,other in relationships:
					#print(other,"->",new[old.index(other)])
					cased_index_set.indices[n].add_relationship(relate, new[old.index(other)])
