...
from qode.SCF_Code.matrix_operation import printline,build_same_np_mat,build_size_mat,printnpline

def diagonalize(M):
    'diagonalize a matrix, returns E and Unitary Matrix'
    # M:Python Matrix; E,U: numpy Matrices
    M_np = np.matrix(M)
    E,U = np.linalg.eig(M_np)
    return E,U
...
def chbasis_main(S_c,T_c,N_c,V_c,num_e):
    ...
    print("ON T matrix dimension =",len(T_on))
    print("ON N matrix dimension =",len(N_on))
    print("ON V matrix dimension =",len(V_on))
    
    Test = test_orthonormal(U_on,S_c)
    print("CHECK POINT FOR ORTHOGONALIZATOIN (Matrix should be identity matrix)")
    printnpline(Test)
