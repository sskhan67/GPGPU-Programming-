...
np.set_printoptions(precision=8,linewidth=270,threshold=np.nan)
...
def sorted_vector_rebuild_spin(U):
    ...
    #
    alpha_column = np.zeros((num_spatial_orb,1))  
    beta_column  = np.zeros((num_spatial_orb,1))  
    #
    for i in range(2*num_spatial_orb):
        column_sum = 0.0
        for j in range(num_spatial_orb):
            column_sum += abs(U[j,i])

        if column_sum != 0.0:
            alpha_column = np.concatenate((alpha_column, U[:num_spatial_orb,i] ), axis=1)
        else:
            beta_column  = np.concatenate((beta_column,  U[num_spatial_orb:,i]),  axis=1)

    alpha_block = alpha_column[:,1:]
    beta_block  = beta_column[:,1:]

    alpha_block = np.concatenate( (alpha_block, np.zeros((num_spatial_orb,num_spatial_orb)) ), axis=0 )
    # print(alpha_block)
    # print("BETA TOP")
    # print(alpha_block[num_spatial_orb:,:])
    # print("BETA BOTTOM")
    # print(alpha_block[:num_spatial_orb:,:])
    beta_block  = np.concatenate( (np.zeros((num_spatial_orb,num_spatial_orb)), beta_block), axis=0)
    # print("Beta Block")
    # print(beta_block)
...
def scf_cycle(E1,U1,T0,N0,V0,alpha_e,beta_e):
    ...
    print("CHECKING ORTHONORMALITY IN EVERY SCF CYCLE...")
    # print(E2)
    # print("=" * 40)
    # print(U2.T * F1 * U2)
    # print("=" * 40)
...
def scf_main(U_on, h_on, T_on, N_on, V_on, num_e, multi, threshold, NuRepulE):

    NRE = NuRepulE
    T_sp, N_sp, V_sp = build_spin.spin_main(h_on, N_on, V_on)
    N_sp.fill(0) # DIRTY CODE

    alpha_e,beta_e = ab_spin(num_e,multi)
    print('# of alpha e-:',alpha_e)
    print('# of beta e-:',beta_e)
    # Initial Guess: F(-1) = T_on + N_on
    print("First Fock = T + N")
    print(T_sp + N_sp)
    ...
    # U_cumu_up  = np.concatenate((U_on, np.zeros(U_on.shape) ), axis=1)
    # U_cumu_low = np.concatenate((np.zeros(U_on.shape), U_on ) , axis=1)
    # U_cumu = np.concatenate((U_cumu_up, U_cumu_low), axis=0)
