...
def experiment(mat,vec,thresh,psize, oscillators, states, coupling):
...
for num_OHs in [3, 4, 5, 6]:
	for states in [3, 4, 5, 6]:
		for coupling in [0.001, 0.01, 0.1]:
			H_param["states_per_oscillator"] = states
			number_of_oscillators            = num_OHs
			H_param["mass"]         = number_of_oscillators * [ H_param["mass"][0] ]
			H_param["k_OHconstant"] = number_of_oscillators * [ H_param["k_OHconstant"][0] ]
			H_param["k_coupling"]   = number_of_oscillators * [ coupling ]
			...
			V = I[:,0]
			V[0,0] = 1
			f = open('results_final_5.out', 'a')
			f.write( experiment(M,V,1e-3,10, num_OHs, states, coupling) )
			f.close()
