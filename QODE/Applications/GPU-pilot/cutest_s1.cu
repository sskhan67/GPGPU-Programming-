...
extern __shared__ volatile double sdata[256];
...
if (freevariables==0) // will not execute cudaFree and host free 
{
...
if (cudaResult != cudaSuccess)
{
 ...
 //cudaMemcpy(h_Hr,d_Hr,sizeof(double)*n_orb1*n_orb1*n_orb1,cudaMemcpyDeviceToHost);
...
count++; // increament count variable 
