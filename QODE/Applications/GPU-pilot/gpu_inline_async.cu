...
void dimer_1min1pls_loop(PyInt n_elem, Double** H, PyInt* i, PyInt* j, PyInt* dim, PyFloat* sign, PyInt* n_orb1_in, PyInt* n_orb2_in, Double** Rc1,  Double** Rcca1, Double** Ra2, Double** Rcaa2, Double** h, Double** V1, Double** V2)
{
    ...
    for(int n=0; n<n_elem; n++) {
        ...
        double tmp = 0.0;
        gpuErr(cudaMemcpyAsync(d_Rcca1,  Rcca1[n],   sizeof(double)*N_Rcca1, cudaMemcpyHostToDevice, streams[0]));
        gpuErr(cudaMemcpyAsync(d_Rcaa2,  Rcaa2[n],   sizeof(double)*N_Rcaa2, cudaMemcpyHostToDevice, streams[1]));
        gpuErr(cudaMemcpyAsync(d_Rc1,    Rc1[n],     sizeof(double)*N_Rc1,   cudaMemcpyHostToDevice, streams[2]));
        gpuErr(cudaMemcpyAsync(d_Ra2,    Ra2[n],     sizeof(double)*N_Ra2,   cudaMemcpyHostToDevice, streams[3]));
        gpuErr(cudaMemcpyAsync(d_h,      h[n],       sizeof(double)*N_h,     cudaMemcpyHostToDevice, streams[4]));
        gpuErr(cudaMemcpyAsync(d_V1112,  V1[n],      sizeof(double)*N_V1112, cudaMemcpyHostToDevice, streams[5]));
        gpuErr(cudaMemcpyAsync(d_V1222,  V2[n],      sizeof(double)*N_V1222, cudaMemcpyHostToDevice, streams[6]));
        for(int k=0; k<7; k++) {
