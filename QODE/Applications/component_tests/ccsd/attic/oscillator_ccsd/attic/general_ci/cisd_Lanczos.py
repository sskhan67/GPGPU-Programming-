...
def build_vector( num_alpha_elec, num_beta_elecm, num_spin_orb ):
    # This is going to build a new reference vector for CISD.
    #
    cisd_config = generate_config.generate_CISD_configs(num_alpha_elec, num_beta_elecm, num_spin_orb, 'CISD')
    cisd_state  = state.state(cisd_config)
    cisd_state.coeffs[0] = 1.0
    return cisd_state

def build_matrix( E_0, F_mat, V_mat ):
    # HF energy, Fock Matrix in spin orbitals, Repulsion Matrix in spin orbitals
    #
    return hamiltonian.hamiltonian( E_0, F_mat, V_mat )
...
def experiment(mat,vec,thresh):
    ...
    H     = simple_cisd.operator(mat)
    guess = simple_cisd.vector(vec)
    print("vector dimension =",vec.get_coeffs().shape[0])
