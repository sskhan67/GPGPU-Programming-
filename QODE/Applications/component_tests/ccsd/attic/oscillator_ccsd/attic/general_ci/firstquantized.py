...
def index_to_excitation(an_index):
	# 
	# VERY DIRTY CODE, ONLY WORKS FOR 28 BY 28 He CISD CODE
	#
	i = an_index
	if i == 0:
		return "REF"
...
num_alpha_virt_orb = num_spatial_orb - num_alpha_elec
num_beta_virt_orb  = num_spatial_orb - num_beta_elec
...
for i_x in range(cisd_vec_ref_len):
	# Loop y starts
	# state_x = copy.deepcopy(state_obj)
	# state_x.coeffs[x_idx] = 1.0
	#
	#
	y_idx = 0
	for i_y in range(cisd_vec_ref_len):
		# print(x_idx,y_idx)
		# state_y = copy.deepcopy(state_obj)
		# state_y.coeffs[0] = 1.0
		# new_state_y = hamiltonian.hamiltonian( state_y, E_0, F_mat, V_mat, num_alpha_elec, num_beta_elec, num_spin_orb )
		# new_state_y.print_info()
		# H_2nd += [ state_dot(state_x, new_state_y ) ]
		#
		y_idx += 1
		#
		#
...
np.set_printoptions(precision=2,linewidth=270,threshold=np.nan)
# print("FIRST Quantization")
# print(H_1st)
print("Second Quantization")
print(H_2nd)

H_2nd = np.array(H_2nd).reshape((size,size))
H_2nd = np.matrix(H_2nd)

np.save("second_quantized_mat.npy",H_2nd)

E,U = np.linalg.eigh(H_2nd)
print("2nd Quantized LOWEST EIGEN VALUE =",E[0])
