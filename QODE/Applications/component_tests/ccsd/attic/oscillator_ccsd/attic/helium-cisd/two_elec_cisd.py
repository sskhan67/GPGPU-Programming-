...
E_0, F_mat, V_mat, num_alpha_elec, num_beta_elec, num_spatial_orb = scf_routine.scf_main("he.in", 1e-6)
...
for i_x in range(cisd_vec_ref_len):
	...
	for i_y in range(num_occ_orb):
		for j_y in range(num_virt_orb):
			...
			# 
			# Sum over Fock
			h1st_temp = F_mat[ diff_orb1[0], diff_orb2[0] ]
...
H_1st = np.reshape(H_1st,(28,28))
H_1st = np.matrix(H_1st)
E1st, V1st = np.linalg.eigh(H_1st)
print("1st Quantization Lower E = ", E1st[0])


# 
#  2nd Quan w/ Numpy  : Lowest Eigenvalue = -66.36873559
#  2nd Quan w/ Lanczos: Lowest eigenvalue = -66.3685996194
# 
# 
