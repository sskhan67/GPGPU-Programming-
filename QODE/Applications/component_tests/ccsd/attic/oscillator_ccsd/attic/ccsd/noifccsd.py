...
def omega0_op_term_onto_HF(op_list_obj, t_amp_obj ,num_occ_orb, num_virt_orb, occ_orbs, virt_orbs, h_mat, V_mat):
	...
	for i in range(len(op_coeffs)):
		# Each term in the long expressions, accumulate them into return_state
		# print(op_coeffs[i], operators[i])
		# # temp_state, excitation_idx_list, t_amp_obj, num_occ_orb, num_virt_orb, occ_orbs, virt_orbs, h_mat, V_mat
		temp_state = operators[i][-1](hf_state, [], t_amp_obj, num_occ_orb, num_virt_orb, occ_orbs, virt_orbs, h_mat, V_mat)
		#
		#
		for j in  reversed(range(len(operators[i])-1)):
			# Each operator in current term, accumulate them into temp_state
			# Now act on states.
			#
			temp_state = operators[i][j]( temp_state, [], t_amp_obj, num_occ_orb, num_virt_orb, occ_orbs, virt_orbs, h_mat, V_mat) 
		return_state.add_to(op_coeffs[i], temp_state)
	#
	#
	return return_state
...
def omega0_mat_parallel(t_amp_obj, H, T, num_alpha_elec, num_beta_elec, num_spin_orb, h_mat, V_mat):
	#
	#
	num_occ_orb, num_virt_orb, alpha_orbs, beta_orbs, alpha_virt_orbs, beta_virt_orbs =\
		generate_config.get_occ_virt_orbs( num_alpha_elec, num_beta_elec, num_spin_orb )
...
def omega1_op_term_onto_HF(omega1_list, t_amp_obj, excitation_idx_list, num_occ_orb, num_virt_orb, occ_orbs, virt_orbs, h_mat, V_mat):
	...
	for op_list_obj in omega1_list:
		...
		for i in range(len(op_coeffs)):
			...
			# print(op_coeffs[i], operators[i])
			# temp_state, excitation_idx_list, t_amp_obj, num_occ_orb, num_virt_orb, occ_orbs, virt_orbs, h_mat, V_mat
			temp_state = operators[i][-1](hf_state, excitation_idx_list, t_amp_obj, num_occ_orb, num_virt_orb, occ_orbs, virt_orbs, h_mat, V_mat)
			#
			#
			for j in  reversed(range(len(operators[i])-1)):
				# Each operator in current term, accumulate them into temp_state
				...
				temp_state = operators[i][j](temp_state, excitation_idx_list, t_amp_obj, num_occ_orb, num_virt_orb, occ_orbs, virt_orbs, h_mat, V_mat) 
			#
			#
			return_state.add_to(op_coeffs[i], temp_state)
	#
...
if __name__ == "__main__":
	...
	t0_start = time.time()
	omega0_vector = omega0_mat_parallel(t_amp_obj, H, T, num_alpha_elec, num_beta_elec, num_spin_orb, h_mat, V_mat)
	t0_end   = time.time()
	...
	E_CCSD = ccsd_energy( t_amp_obj , H, T, num_alpha_elec, num_beta_elec, num_spin_orb, h_mat, V_mat)
	...
	while not converged:
		...
		t_amp_obj.update_coeffs( t_amp_obj.get_coeffs() + dt_amps_long.A1 )
		print("New t Amplitudes Done.")
		E_CCSD_new = ccsd_energy( t_amp_obj, H, T, num_alpha_elec, num_beta_elec, num_spin_orb, h_mat, V_mat)
