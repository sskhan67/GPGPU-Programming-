...
# CCSD module
from fast import ccsd

# Oscillator System Imports
from attic.oscillator_ccsd.old_qode.generate_ho_config import diag_rec_h0_mat, diag_rec_v_matrix, get_lowest_analytical, get_rec_obj
...
def main(n_ho, n_state, n_mol):
	...
	h_mat_object = diag_rec_h0_mat(all_mol_eigenvalues)
	V_mat_object = diag_rec_v_matrix(rec_num_states, v_mat_list)
	h_mat = numpy.zeros( shape=(n_orbs, n_orbs) )
	V_mat = numpy.zeros( shape=(n_orbs*n_orbs, n_orbs*n_orbs) )

	for p in range(n_orbs):
		for q in range(n_orbs):
			h_mat[p,q] = h_mat_object(p,q)

	from_mol = []
	for i in range(len(rec_num_states)):
		from_mol += [i for n in range(rec_num_states[i])]

	# out.log(indent("from molecule list   = ", from_mol))

	mol_loc = []
	for n in rec_num_states:
		mol_loc += [i for i in range(n)]

	# out.log(indent("molecule local index = ", mol_loc))


	V_mat = numpy.zeros((n_orbs*n_orbs,n_orbs*n_orbs))
	for p in range(n_orbs):
		for q in range(n_orbs):
				...
				for s in range(n_orbs):
					if from_mol[p] == from_mol[r] and from_mol[q] == from_mol[s]:
						if from_mol[p] < from_mol[q]:
							V_mat[ p*n_orbs+q , r*n_orbs+s ] = V_mat_object(from_mol[p], from_mol[q], mol_loc[p], mol_loc[q], mol_loc[r], mol_loc[s])
						elif from_mol[p] > from_mol[q]:
							V_mat[ p*n_orbs+q , r*n_orbs+s ] = V_mat_object(from_mol[q], from_mol[p], mol_loc[q], mol_loc[p], mol_loc[s], mol_loc[r])
	...
	out.ccsd = ccsd.main(h_mat, V_mat, h_mat, 0.0, occ_orbs, vrt_orbs, textlog=out.log.sublog(), resources=resources)
...
if __name__ == "__main__":
	n_ho = 8
	for n_mol in [2]:
		for n_state in [10,15]:
			# for n_mol, n_state in [[2,3]]:
			try:
	...
	# 8 ho 10 states 2 mol: CCSD thresh=1e-6
	# TOTAL ENERGY        = 9.57794024318
	# ANALYTICAL SOLUTION = 9.577940238168155

	# 8 ho 15 states 2 mol: 
	# TOTAL ENERGY        = 9.57794023964
	# ANALYTICAL SOLUTION = 9.577940238168155

	# 8 ho 10 states 2 mol: CCSD thresh=1e-10
	# TOTAL ENERGY        = 9.57794024318
	# ANALYTICAL SOLUTION = 9.577940238168155

	# 8 ho 15 states 2 mol: CCSD thresh=1e-10
	# TOTAL ENERGY        = 9.57794023964
	# ANALYTICAL SOLUTION = 9.577940238168155
