		...
		for n in range(num_mol): 
			if m < n:
				for a in range(Nv[m]):
					for b in range(Nv[n]):
						for c in range(Nv[n]):
							H.ExFv[ H.ExFv_starters[m*num_mol+n] + a*Nv[n]**2 + b*Nv[n] + c ] += V_mat( m, n, 0, b+1, a+1, c+1 )
			if m > n:
				for a in range(Nv[m]):
					for b in range(Nv[n]):
						for c in range(Nv[n]):
							H.ExFv[ H.ExFv_starters[m*num_mol+n] + a*Nv[n]**2 + b*Nv[n] + c ] += V_mat( n, m, b+1, 0, c+1, a+1 )
	...
	for m in range(num_mol):
		for n in range(num_mol):
			if m < n:
				for a in range(Nv[n]):
					for b in range(Nv[n]):
						H.FoFv[ H.FoFv_starters[m*num_mol+n] + a*Nv[n] + b ] += -V_mat( m, n, 0, a+1, 0, b+1 )
			elif m > n:
				for a in range(Nv[n]):
					for b in range(Nv[n]):
						H.FoFv[ H.FoFv_starters[m*num_mol+n] + a*Nv[n] + b ] += -V_mat( n, m, a+1, 0, b+1, 0 )
			...
			if m > n:
				for a in range(Nv[m]):
					for b in range(Nv[m]):
						for c in range(Nv[n]):
							for d in range(Nv[n]):
								H.FvFv[ H.FvFv_starters[m*num_mol+n] +  a*Nv[m]*Nv[n]**2 + b*Nv[n]**2 + c*Nv[n] + d ] += V_mat( n, m, c+1, a+1, d+1, b+1 )
		...
		for n in range(num_mol): 		
			if m < n:		
				for a in range(Nv[m]):
					for b in range(Nv[m]):
						for c in range(Nv[n]):
							H.FvDx[ H.FvDx_starters[m*num_mol+n] + a*Nv[m]*Nv[n] + b*Nv[n] + c ] += -V_mat( m, n, a+1, c+1, b+1, 0 )
			elif m > n:
				for a in range(Nv[m]):
					for b in range(Nv[m]):
						for c in range(Nv[n]):
							H.FvDx[ H.FvDx_starters[m*num_mol+n] + a*Nv[m]*Nv[n] + b*Nv[n] + c ] += -V_mat( n, m, c+1, a+1, 0, b+1 )
