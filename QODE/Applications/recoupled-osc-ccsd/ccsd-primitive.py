...
# maximum length of a job in seconds (5 hours)
...
def main(mol_obj, rec_num_states, out, analytical_value, resources):
	...
	try:
		E0, H.K[0] = H.K[0], 0.
		out.ccsd.energy = E0 + CCSD(H, T, invF, BCH, space_traits, out.ccsd.log.sublog(), thresh=1e-10)
...
if __name__ == "__main__":
	...
	for n_mol,max_states in [         [2,10], [3,10], [4,10], [5,10], [6,10], [7,10], [8,10], [9,10],[10,10],
		...
		if analytical_value is None:  out.log("+++++++++++++++++++++ %d ho %d mol           ++++++++++++++++++++++++++++++++++++++\nFAILED!!" %(n_ho, n_mol))
			...
			if n_state < max_states:
				...
				while n_state <= max_states:
					...
					n_state += 1
	out.log.write('primitive_{:.1f}.out'.format(separation))
