...
def get_n_mol_obj(
    ...
    print("CHECK COUPLING MATRIX")
    for line in coupling_mat:
        print(line)
    
    new_ld = len(coupling_mat)
    print("DIMENSION =",new_ld)
...
def plot_main(
    ...
    while run:
        ...
        try:
            ...
            #H = nbody_hamiltonian_func.rec_obj_build_full_hamiltonian( rec_obj )
            ...
            if high_accu_limit <  percent_error <  low_accu_limit:
                ...
                if rec_state_history[-1] == max(rec_state_history):
                    ...
                    for i in range(num_oscillator*num_molecule):
                        rec_num_states[i] *= 2
                else:
