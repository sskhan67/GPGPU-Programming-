...
def frozen_core_Sz0_fci_states(num_spin_orbs, num_elec, num_core_elec):
	...
	# np.save('new_fci_states.npy',fci_states)
...
def main(prefix, h_mat_file, V_mat_file, num_elec, num_core_elec=None, frozen_core=False, force_sz0=False):
	...
	np.save(prefix + '_fci_ham_mat.npy', H_fci)
	print("Saving FCI Configs onto Disk...")
	np.save(prefix+"_all_states_int64.npy", np.array(fci_states, dtype=np.int64))
	import numpyLanczos
	...
	# np.save(prefix+"C_FCI.npy", C)
	# np.save(prefix+"E_FCI.npy", E)
	return H_fci
